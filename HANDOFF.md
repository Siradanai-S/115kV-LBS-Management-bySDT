...
- **ฐานข้อมูลวัสดุ (Item Catalog):** ตาราง `item_catalog` (epicor_code UNIQUE/description/ium · RLS อ่านทุกฝ่าย เขียนเฉพาะ dev) · Setting → การ์ด `CatalogSettings` เพิ่ม/แก้ inline (EditText)/ลบ + **Import Excel = upsert ตาม epicor_code** (หัวตารางยืดหยุ่น, dedupe ในไฟล์, ไม่ลบของเดิม) + **Export Excel** (round-trip ได้) · ฟอร์ม BOM ช่อง Epicor Code = `CodeSearch` (ค้น code/description จาก `state.catalog` → เลือกแล้ว auto-fill Description+IUM · พิมพ์เองได้) · db: `addCatalogItem`/`upsertCatalog` · loadAll ห่อ try/catch (DB ที่ยังไม่รัน schema ใหม่ไม่พัง)
