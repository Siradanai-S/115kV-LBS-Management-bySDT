...
- **ใบเบิกติดตั้ง (ออกที่ฝ่ายโครงการ):** Tab "เตรียมส่งมอบ Service" (`project-handover`) → `HandoverSlip` เลือก BOM+จำนวน+แผน start/end → `handleSendHandover` ตัดสต็อก OUT 'ติดตั้งงาน' (สิทธิ์เบิกที่ Project) + สร้าง `service_plans` (wd_no/lines/plan_start/plan_end) · Service inbox: เลือก Job/ทีมที่ว่าง (`memberBusy`) → `handleAckPlan(id, teamIds)` ack + ลงคิวปฏิทิน · `printWithdrawal()` ปริ้นใบเบิกไปตรวจ WH · DO ลงนามที่ Service · **คลัง = รับเข้าตาม PO + โอนข้าม Stock เท่านั้น** (manual OUT เหลือ คืนผู้ขาย/ชำรุด)
