...
- **Service flow แบบขั้นตอน (auto-advance):** หน้า Service = การ์ดงานต่อใบเบิก (`ServiceJob`) 3 ขั้น กดแล้วเลื่อนอัตโนมัติ — (1) รับงาน (รับใบเบิก+แผน+ปริ้น D/O+Handover ผ่าน `printServiceDoc('wd'|'do'|'handover')`) → `handleSvcReceive` (2) จัดทีม 1-2 คน + actual Start/End → `handleSvcAssign` (ลงคิวปฏิทินตามเวลาจริง) (3) ส่งมอบ + แนบไฟล์ PDF/รูป → `handleSvcDeliver` (ลงนาม DO + notify('do')) · stage derive จาก acked/team+actual/delivered · auto-complete service task ตามขั้น (`completeSvcTask`) · plan fields: actual_start/actual_end/proof_name/proof_url/delivered · เพิ่มสมาชิก: `emp_code` (รหัสพนักงาน) + role กรอกเอง (service_team.emp_code)
- **Project → คลังสินค้า (Inventory) [ปรับใหม่]:** ลบ tab "เตรียมส่งมอบ Service" + ลบเมนู Inventory top-level → รวมเป็น tab `project-inventory` ใต้ Project (`ProjectInventory`) · **รับเข้าอัตโนมัติ**: เมื่อ `handleUpdateBom` ตั้ง po_status='Delivered' → addMovement IN เข้า project_id (เต็มจำนวน BOM, กันซ้ำด้วย note `BOM#id`) · **เบิกอ้าง SR-No.** (`WithdrawSR` + `handleWithdrawSR`): เลือก SR → โชว์ลูกค้าทั้งใบ → เลือกรายการ/จำนวนจาก onhand + plan start/end → ตัดสต็อก OUT 'ติดตั้งงาน' + `addPlan(sr_id, sent:true)` = รอส่งมอบ Service (เบิกซ้ำได้หลายครั้งต่อ SR) · `service_plans.sr_id` (schema) · helper `PLAN_STATUS` · component `Inventory`/`HandoverSlip` เดิม = dead code (ไม่ได้ route แล้ว)
...
- **Dashboard:** KPI + คอขวด + เตือนส่งมอบ + ตารางภาพรวม Stock (เอากราฟ S-Curve/รายเดือนออกแล้วตามคำขอ — component `SCurveChart`/`MonthlyBars` ยังนิยามไว้แต่ไม่เรียกใช้)
