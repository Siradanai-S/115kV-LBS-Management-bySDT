...
- **Gate ปิดงานนับเฉพาะใบอนุมัติ (fix):** `phaseDataGate`(service) + RPC `handoff_project` กรอง `sent && approved` ตรงกับ `allPlansDelivered` (ใบรออนุมัติ DM ไม่บล็อกปิดงาน) · ถอด backfill `UPDATE approved=TRUE` ออกจาก schema.sql (รันซ้ำจะ auto-approve ใบค้างโดยไม่ตัดสต็อก — ห้ามใส่กลับ)
- **ฐานข้อมูลวัสดุ (Item Catalog):** ตาราง `item_catalog` (epicor_code UNIQUE/description/ium · RLS อ่านทุกฝ่าย · **เขียน dev + project**) · Setting → การ์ด `CatalogSettings` เพิ่ม/แก้ inline (EditText)/ลบ + **Import Excel = upsert ตาม epicor_code** (หัวตารางยืดหยุ่น, dedupe ในไฟล์, ไม่ลบของเดิม) + **Export Excel** (round-trip ได้) · **เมนู Project → "📚 ฐานข้อมูลวัสดุ"** (`page='project-catalog'`) ใช้ `CatalogSettings` ตัวเดียวกัน prop `canEdit` (project/dev แก้ได้ · ฝ่ายอื่นดู+Export อย่างเดียว) · ฟอร์ม BOM ช่อง Epicor Code = `CodeSearch` (ค้น code/description จาก `state.catalog` → เลือกแล้ว auto-fill Description+IUM · พิมพ์เองได้) · db: `addCatalogItem`/`upsertCatalog` · loadAll ห่อ try/catch (DB ที่ยังไม่รัน schema ใหม่ไม่พัง)
- **Setting การ์ดพับได้ (เริ่มต้นซ่อน):** ผู้ใช้งาน&สิทธิ์ (`showUsers` · แถบเตือนรออนุมัติยังโชว์เสมอ กดแล้วกาง) / จัดการข้อมูลทุกฝ่าย CRUD (`showCrud`) / การแจ้งเตือน LINE-Email (state `show` ใน `NotifSettings`)
- **Project Job No. ต่อลูกค้า + Budget ต่อ Job (restructure):**
  - `projects.jobs` (JSONB) = `{ [job_no]: {cust_id, sell_price, cost_budget} }` · สร้าง Job = ปุ่ม "สร้าง Project Job No. (เลือกลูกค้า)" ในหน้า BOM list (เลข auto `JOB-ฐาน-n` แก้ได้ · กันซ้ำ) · ลบ Job ได้เมื่อยังไม่มีครั้ง
  - **Budget ย้ายจาก Stock → Job:** บล็อก 💰 ใน `JobGroupCard` — กรอก ราคาขาย + ต้นทุน(งบ) → กำไร=ขาย−ต้นทุน · ต้นทุนคงเหลือ=ต้นทุน−มูลค่า BOM ของ Job (ติดลบ=แดง+เตือน) · การ์ด Stock เหลือ ชื่อ/Stock No./มูลค่า BOM รวม (ลบ งบประมาณรวม/`budgetLeft`)
  - **popup สร้าง Stock ตัดช่อง Job No. ออก** (ส่ง jobNo:'' · RPC เดิมรับได้) · **gate project→purchasing เลิกบังคับ `projects.job_no`** ทั้ง client (`phaseDataGate`) และ RPC `handoff_project` (schema.sql)
  - เมนู Project เปลี่ยน: กิ่ง "Project Job No. (สร้าง Project Stock No.)" (= หน้า `project` เดิม Import SR) + rename "Purchase Requisition (Job No.)" → **"Project Job No. (BOM list)"**
  - `jobGroups` เริ่มจาก keys ของ `projects.jobs` (Job ว่างได้) แล้วเติมกลุ่มจาก rounds (legacy `bom_rounds[n].job_no` ยังทำงาน) · seedDemo ใส่ jobs ครบ 4 stock
- **Serial LVB/OM ต่อ LBS 1 ตัว (ย้ายจาก BOM → ลูกค้า):** `customers.serials` JSONB `[{lvb,om}]` ยาว = lbs_qty · หน้า SR inbox (Project) คอลัมน์ Serial + ปุ่ม "✎ กรอก Serial" เปิด modal `SerialTable` (EditText commit-on-blur) · **gate: กรอกครบทุกตัวของทุกลูกค้าที่เลือกก่อนจึงกด Import → สร้าง Stock ได้** (`serialsOk`/`serialDone` + แถบแดงบอกใครยังขาด) · แก้ภายหลังได้ในกล่อง Job (BOM list) section "🔢 Serial LVB/OM" · **ตัดช่อง Serial ออกจากฟอร์ม BOM แล้ว** (คอลัมน์ bom_items.serial_* คงอยู่ อ่านได้ผ่าน RowEditor) · RLS ใหม่ `p_cust_upd_project` (project UPDATE customers ได้) · popup สร้าง Stock ตัด BOM No. ออก (เหลือ ชื่อ/LBS/Stock No.) · เมนูกิ่งแรก rename → "สร้าง Project Stock No."
- **schema ที่ต้องรันสะสม:** projects.bom_rounds · customers.project_name/contract_file_* · bom_items.cust_id · service_plans.checkin_*/cust_id/approved/approved_by/has_po/advance_payment · RLS p_proj_del(+project) / p_plans_upd(+executive) / p_notif read-all · **item_catalog (ตาราง+RLS)** · handoff_project (service gate กรอง approved · project gate เลิกบังคับ job_no) · **projects.jobs (JSONB)** · **customers.serials (JSONB) + RLS p_cust_upd_project**
